...
class Zhui403Spider(scrapy.Spider):
    '''user info'''
    name = 'zhui403'
    ...
    def start_requests(self):
        ...
        try:
            with open('./data/uinfo403.csv', 'r+') as f:
                reader = csv.reader(f)
                for im in reader:
        ...
        print("完成ui403    time:" + str(int(T / 3600)) + "h " +
        ...
        cur = time.time()
        T = int(cur - self.start)
        print("完成new task    time:" + str(int(T / 3600)) + "h " +
              str(int(T / 60) % 60) + "m " + str(T % 60) + "s......")
        reqs = []
        print('new task num:{}'.format(len(self.userList)))
