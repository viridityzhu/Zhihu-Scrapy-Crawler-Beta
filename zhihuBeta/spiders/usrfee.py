# -*- coding: utf-8 -*-
import scrapy
from zhihuBeta.items import UserFolloweeItem
import json
import csv
import os
import math
...
class ZhufeSpider(scrapy.Spider):
    '''user followee'''
    name = 'zhufe'
    ...
    def start_requests(self):
        ...
        try:
            ...
            open('./tempData/ReallyFinishedUe.csv', 'w+')
        self.userList = [
            item for item in set(userListF) if item not in set(finishedUe)]
    ...
    def parse(self, response):
        ...
        if totals > 5000:
            self.logger.warning("Big V:{}, total:{}, page rate:{}".format(
                u1, totals, pageToTurn))
            # ----------获取话题user followee的url以及其他信息---------
        ...
        if not isEnd:
            ...
            # 判断有没有数完所有人
            fList = []
            with open('./tempData/finishedUe.csv', 'r+') as f:
                reader = csv.reader(f)
                for im in reader:
                    fList.append(im[0])
